...
class Card:
    ...
    emo_CLUBS = "U0002663" # эмоджи Крести
    emo_HEARTS = "U0002665" # Черви
    emo_DIAMONDS = "U0002666" # Буби
...
class Game21:
    def __init__(self, deck_count=1):
        new_pack = self.new_pack(deck_count) #в конструкторе создаем новую пачку из deck_coutn-колод
        if new_pack is not None:
            self.pack_card = new_pack #сформированная колода
            self.remaining = new_pack["remaining"], #кол-во оставшихся карт в колоде
            self.card_in_game = [] #карты в игре
            self.arr_cards_URL = [] #URL карт игрока
            self.score = 0 #очки игрока
            self.status = None #статус игры, True-тгрок выиграл, False-проиграл, None- игра продолжается
    ...
    def get_cards(self, card_count=1):
        ...
        if new_cards["success"] != True:
            return False
        self.remaining = new_cards["remaining"] #обновим в классе кол-во оставшихся карт в колоде
        arr_newCards = []
        for card in new_cards["cards"]:
            card_obj = Card(card) #создаем объукты класса CFrd и добавляем их в список карт у игрока
